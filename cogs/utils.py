...
class utils(commands.Cog):
    ...
    async def status(self,interaction:Interaction,
        ...
        if type == "Listening":
            try:
                await self.client.change_presence(activity=nc.Activity(type=nc.ActivityType.listening, name=f"{status}"))
            ...
            try:
                await self.client.change_presence(activity=nc.Streaming(name=f"{status}",url="https://www.youtube.com/watch?v=dQw4w9WgXcQ")) # you know where the link leads(hopefully)
                await interaction.response.send_message(embed=embed)
            except:
                await interaction.response.send_message("An error occured")
        else:
        ...
        if str(message.guild.id) in whitelisted_ids and "damn" not in message.content: # check if guild is whitelisted
            ...
            if int(msg_predict_prob) > 82 : # if message contains profanity
                ...
                if channel is not None:
                    embed = nc.Embed(title="Message Deleted",description=f"{message.author.mention} has been deleted for containing a profanity of {msg_predict_prob}%")
