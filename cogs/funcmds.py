    ...
    async def flip(self,
     ...
     reason=SlashOption(
      ...
      required=False)
        ...
        embed = nc.Embed(title=f"The 🪙 flipped to {random.choice(coin)}" + suffix + "!") # choose random choice and embed and reply
        embed.add_field(
            name="Your choice",
            value=f"{side}"
            )
    ...
    class SnipePagination(nextcord.ui.View):
      ...
      async def check_author(self, ictx):
        if ictx.user != self.author:
          await ictx.response.send_message("Not your command pal, make your own using the command `snipe` :v",ephemeral=True)
          return False
        else:
          return True
      ...
      def format_page(self,page):
        ...
        if self.type == "snipe":
          ...
          embed.add_field(name="Message", value=message.content if message.content != "" else "No message content", inline=False)
